...
sub event_publish {
    ...
    foreach my $descendant (@descendants) {
        my $boolean = (
            $descendant->grant_user_privileges(
                grantee        => XIMS::PUBLICUSERID(),
                privilege_mask => $privmask,
                grantor        => $ctxt->session->user->id()
                )
                and $descendant->publish( User => $ctxt->session->user )
        );
        unless ($boolean) {
            XIMS::Debug( 3,
                "could not grant to descendant with doc_id "
                    . $descendant->document_id() );
        }
        else {
            $granted++;
        }
    ...
    if ( $granted == scalar @descendants ) {
        $ctxt->session->message(
            __x("Object '{title}' together with {num} related objects published.",
