		...
		<html>
			...
			<body>
				...
				<div id="tab-container" class="ui-corner-top">
						<xsl:call-template name="table-move"/>
					</div>				
...
<div id="table-container" class="ui-corner-bottom ui-corner-tr">				
	...
	<xsl:template match="targetparents/object|target/object">
      / <a class="">
			<xsl:attribute name="href"><xsl:value-of select="concat($xims_box,$goxims_content,'?id=',/document/context/object/@id,';move_browse=1;to=',@id)"/><!--<xsl:call-template name="rbacknav_qs"/>--></xsl:attribute>
