...
<xsl:template name="tb_question_actions">
  ...
  <table>
    <tr>
      ...
      <td>
          <xsl:if test="(name() = 'question') and (name(..) = 'questionnaire') and not(@edit)" >
                <input
                    type="image"
	    ...
	    <tr class="answer-header">
	      ...
	      <td>
	        <select tabindex="10" name="answer_{$position_long}_select">
	          ...
	          <input type="button" value="Remove from Selection" onclick="removeSelection(answer_{$position_long}_select);" />
	        <br />
	        <input type="text" class="text" size="40" tabindex="10" name="answer_{$position_long}_add" value = "" />
	          <input type="button" value="Add to Selection" onclick="addSelection(answer_{$position_long}_add,answer_{$position_long}_select);" /><br />
...
<xsl:template name="questionnaire_actions">
  <tr>
    <td valign="top" colspan="2">
      <input type="submit" value="Add Question" onclick="eform.edit.value='add_question';eform.qid.value='';return true;" />
      <input type="button" value="Add Tanlist" onclick="genericWindow('{$xims_box}{$goxims_content}?id={@id};contentbrowse=1;to={@id};otfilter=TAN_List;sbfield=eform');" />
...
<xsl:template name="question_title">
  ...
  <xsl:choose>
    <xsl:when test="$edit">
      ...
      <td>
        <textarea tabindex="10" name="question_{$position_long}_title" cols="40" rows="3" class="text"> 
...
<xsl:template name="question_actions">
  ...
  <tr>
    <td>&#160;</td>
    <td valign="top" colspan="2">
      <input type="submit" value="Add Subquestion" onclick="eform.edit.value='add_question';eform.qid.value='{$position_long}';return true;" />
      &#160;
      <input type="submit" value="Add Answer" onclick="eform.edit.value='add_answer';eform.qid.value='{$position_long}';return true;" />
      &#160;
    </td>
  </tr>
</xsl:template>
  ...
  <tr>
    <td valign="top" colspan="2">
      ...
      <xsl:for-each select="body/questionnaire/tanlist">
        ...
        <input type="submit" value="Remove Tanlist" onclick="eform.edit.value='remove_tanlist';eform.qid.value={@id};return true;" />
        <br />
      </xsl:for-each>
    </td>
  </tr>
