...
<xsl:template name="tr-vlkeywords-edit">
    <tr>
        <td valign="top">Existing <xsl:value-of select="$i18n/l/Keywords"/></td>
        <td colspan="2">
    ...
    <tr>
        <td valign="top">Assign new VLKeyword(s)</td>
        <td colspan="2">
            ...
            <xsl:text>&#160;</xsl:text>
            <input type="submit" name="create_mapping" value="Create mapping" class="control" onClick="return submitOnValue(document.eform.vlkeyword, 'Please fill in a value for', document.eform.svlkeyword);"/>
        </td>
    </tr>
</xsl:template>
...
<xsl:template name="tr-vlsubjects-edit">
    <tr>
        <td valign="top">Existing VLSubject(s)<xsl:value-of select="$i18n/l/Subjects"/></td>
        <td colspan="2">
    ...
    <tr>
        <td valign="top">Assign new VLSubject(s)</td>
        <td colspan="2">
            ...
            <xsl:text>&#160;</xsl:text>
            <input type="submit" name="create_mapping" value="Create mapping" class="control"/>
        </td>
    </tr>
</xsl:template>
...
<xsl:template match="keywordset/keyword|subjectset/subject">
    <a href="{$xims_box}{$goxims_content}{$parent_path}?{name()}=1;{concat(name(),'_id')}={id}" target="_blank" title="Browse in a new window"><xsl:value-of select="name"/></a>
    <xsl:text> </xsl:text>
