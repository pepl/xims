...
<xsl:template match="bookmark">
    <tr>
        ...
        <td valign="top">
            <xsl:choose>
                <xsl:when test="stdhome != '1'">
                    <a href="/goxims/bookmark?id={id};setdefault=1;name={$name}"><xsl:value-of select="$i18n/l/set_as"/>&#160;<xsl:value-of select="$i18n/l/default_bookmark"/></a>
                </xsl:when>
                <xsl:otherwise>
        ...
        <td>
            &#160;
            <a href="/goxims/bookmark?id={id};delete=1;name={$name}">
                <img
