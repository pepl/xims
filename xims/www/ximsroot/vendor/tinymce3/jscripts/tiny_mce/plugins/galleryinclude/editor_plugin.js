...
(function() {
	...
	tinymce.create('tinymce.plugins.galleryinclude', {
		init : function(ed, url) {
			...
			// Register commands
			ed.addCommand('mcegalleryinclude', function() {
				//Popup window
				ed.windowManager.open({
					file : url + '/galleryinclude.html',
					width : 380 + parseInt(ed.getLang('galleryinclude.delta_width', 0)),
					height : 380 + parseInt(ed.getLang('galleryinclude.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url
				});
			...
			// Register buttons
			ed.addButton('galleryinclude', {
				title : 'galleryinclude.desc',
				cmd : 'mcegalleryinclude'
			});
			
			//toggle Button State
			ed.onNodeChange.add(function(ed, cm, n, co, o) {
				n = ed.dom.getParent(n, 'span.xml_include');
				cm.setActive('galleryinclude', 0);
				// Activate all
				if(n){
					do{
						//cm.setDisabled('xlink', 0);
						cm.setActive('galleryinclude', 1);
					}
		...
		getInfo : function() {
			return {
				longname : 'Gallery Include Plugin, adapted from advlink-Plugin',
				author : 'Susanne Tober, adapted from moxiecodes advlink',
	...
	// Register plugin
	tinymce.PluginManager.add('galleryinclude', tinymce.plugins.galleryinclude);
})();
