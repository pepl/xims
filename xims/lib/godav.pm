...
sub handler {
    ...
    my $headers_out = [ 'X-Server' => 'XIMS::DAVServer ' . $VERSION,
                        'MS-Author-Via' => 'DAV',];
    ...
    no strict 'refs';
    #my ($status_code, $headers, $content) = &{$method}( $env, $user );
    #use strict 'refs';
    #\# use critic
...
sub options {
    ...
    return [
        ...
        ['DAV' => '1, 2, <http://apache.org/dav/propset/fs/1>',
         'Allow' => 'OPTIONS, GET, HEAD, POST, DELETE, TRACE, PROPFIND, LOCK, UNLOCK, MOVE'],
        undef
        ];
}
...
sub get {
    ...
    if ( $object->object_type->is_fs_container() ) {
        ...
        return [
            200,
            [
             'Content-Type'   => $object->data_format->mime_type() . "; charset=$charset",
             #'Content-Length' => $stat[7],
             'Last-Modified'  => HTTP::Date::time2str( $t->epoch )
            ],
            [$object->body()]
        ];
...
sub put {
    ...
    if ( defined $object ) {
        ...
        HTTP::Exception->throw(403) unless defined $parent and defined $parent->id();

        my $privmask = $user->object_privmask( $parent );
        HTTP::Exception->throw(403) unless $privmask & XIMS::Privileges::CREATE;
    ...
    HTTP::Exception->throw($status_code);
}
...
EOS
            return ['424', ['Content-Type' => 'application/xml; charset="UTF-8"'], [$response]];
        }
...
EOS
            return ['207', ['Content-Type' => 'application/xml; charset="UTF-8"'], [$response]]
            #return (207, $response);
        }
...
sub mkcol {
    ...
    if ( $env->{CONTENT_LENGTH} ) {
        ...
        my $parent = XIMS::Object->new( User => $user, path => $parentpath, marked_deleted => 0 );
        HTTP::Exception->throw(409) unless (defined $parent and defined $parent->id());
...
sub propfind {
    ...
    my $privmask = $user->object_privmask( $object );
    HTTP::Exception->throw(403) unless defined $privmask and ($privmask & XIMS::Privileges::VIEW);
    ...
    return['207', ['Content-Type' => 'application/xml; charset="UTF-8"'], [$dom->toString(1)]]
}
...
EOS
    ...
    return ['200', ['Content-Type' => 'application/xml; charset="UTF-8"',
...
sub copymove {
    ...
    ## no critic (ProhibitNoStrict)
    no strict 'refs';
    HTTP::Exception->throw(403) unless $privmask & &{"XIMS::Privileges::$method"};
    use strict 'refs';
    ## use critic
    $method = lc $method;
    ...
    if ( defined $destination ) {
        ...
        if ( defined $overwrite and $overwrite eq 'T' ) {
            ...
            HTTP::Exception->throw(403) unless $privmask and ($privmask & XIMS::Privileges::DELETE());
            if ( $destination->location ne $object->location ) {
                $privmask = $user->object_privmask( $object );
                HTTP::Exception->throw(403) unless $privmask and ($privmask & XIMS::Privileges::WRITE());
            }
        ...
        HTTP::Exception->throw(403) unless $privmask and ($privmask & XIMS::Privileges::CREATE());
