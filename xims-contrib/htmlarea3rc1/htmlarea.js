...
HTMLArea.Config = function () {
    ...
    this.mozParaHandler = 'dirty'; // set to 'built-in', 'dirty' or 'best'
  ...
  // URL-s for contentbrowse in XIMS
    this.imageURL ="";
    this.linkURL="";
	...
	this.toolbar = [
		...
		[ "justifyleft", "justifycenter", "justifyright", "justifyfull", "separator",
		  ...
		  "killword", "clearfonts", "removeformat", "separator",
	...
	this.btnList = {
		...
		clearfonts: [ "Clear Font Specifications", "ed_clearfonts.gif", false, function(e) {e._clearFonts();} ]
	};
	...
	function createSelect(txt) {
		...
		if (options) {
			...
			for (var i in options) {
				...
				if (typeof HTMLArea.I18N.selectoptions[i] != "undefined") {
	                op.innerHTML = HTMLArea.I18N.selectoptions[i];
				}
				else {
					op.innerHTML = i;
				}
...
HTMLArea.prototype._getFirstAncestor = function(sel, types)
{
  ...
  while(prnt)
  {
    if(prnt.nodeType == 1)
    {
      ...
      {
        if(prnt.tagName.toLowerCase() == types[i]) found = true;
      }
      if(found == true)
      {
        return prnt;
...
// XIMS
HTMLArea.prototype._createLinkX = function(link) {
    var editor = this;
    var a = link;
    editor.insertHTML(a);
    editor.updateToolbar();
...
// Called for insertImage in XIMS
HTMLArea.prototype._insertImageX = function(path,title){
	//alert(path);
    ...
    var img = image;
        if (!img) {
            var sel = editor._getSelection();
            var range = editor._createRange(sel);
            editor._doc.execCommand("insertimage", false,path);
            if (HTMLArea.is_ie) {
...
//XIMS
        ...
        if(this.config.linkURL == ""){
            this._createLink();break;
        }
        else{
            this.insertDlg (this.config.linkURL,0); break;
        }
	    case "popupeditor":
...
//XIMS
        ...
        if(this.config.imageURL == ""){
            ...
            this.insertDlg(this.config.imageURL,0); break;
        }
...
HTMLArea.needsClosingTag = function(el) {
	...
	return el && el.nodeType == 1 && (closingTags.indexOf(" " + el.tagName.toLowerCase() + " ") != -1);
};
		...
		if (HTMLArea.is_ie && root_tag == "head") {
			...
			for (i = 0; i < attrs.length; ++i) {
				...
				if (name != "style") {
					...
					if (typeof a.nodeValue != "undefined" && name != "href" && name != "src" ) {
						//value = root[a.nodeName];
						var tval = a.nodeValue;
						if ( tval.length == 0 ) {
							continue;
						}
						else {
...
// popUps for contentbrowse in XIMS
HTMLArea.prototype.insertDlg = function(url, action, init){
    Dialog(url, action, init, 0);
}
