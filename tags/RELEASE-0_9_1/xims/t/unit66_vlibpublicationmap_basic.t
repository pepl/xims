...
cmp_ok( $publication_id, '>', 0, "Publication created with id $publication_id" );
...
ok( $pubmap );
is( $pubmap->document_id(), $document->document_id(), 'TestPublicationMap has correct document_id' );
is( $pubmap->publication_id(), $publication->id(), 'TestPublicationMap has correct publication_id' );

# fetch it back by document_id
$pubmap = undef;
$pubmap = XIMS::VLibPublicationMap->new( document_id => $document->document_id(), publication_id => $publication_id );
