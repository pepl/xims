    ...
    <xsl:choose>
        ...
        <xsl:when test="$current_question >= 1">
            <form action="?q={$current_question + 1}" method="post">
                ...
                <input type="submit" name="answer" value="Weiter >" />
            </form>
        </xsl:when>
        <xsl:otherwise>
            <div align="center">
...
<xsl:template name="questionnaire_exit">
    <xsl:choose>
        ...
        <xsl:otherwise><span class="questionnaire-intro-public">Vielen Dank, dass sie den Fragebogen beantwortet haben.</span></xsl:otherwise>
    </xsl:choose>
</xsl:template>
