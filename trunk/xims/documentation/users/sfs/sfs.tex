...
\item {Erstellen eines Verzeichnisses mit der \ximsterm{Location}
    \path{departmentlinks}. Die \ximsterm{Location} des Verzeichnisses
    muss zwingend \path{departmentlinks} heißen.}
...
\item {Im nächsten Schritt müssen im soeben erstellten Verzeichnis mit
    der \ximsterm{Location} \path{departmentlinks} all jene
    \ximsterm{URLLink}-Objekte erstellt werden, die als Navigation
    bzw. DepartementLinks zur Verfügung stehen sollen. Die Anleitung
    zum Erstellen von \ximsterm{URLLink}-Objekten kann unter Punkt
    \ref{urllinks} nachgelesen werden. Die \ximsterm{Location} der
    Links muss relativ zum \ximsterm{SiteRoot} sein. Um zum Beispiel
    einen Verweis auf das \otherterm{XIMS}-Objekt mit dem Pfad
    \path{/uniweb/service/ c102} zu erstellen, muss im Eingabefeld
    \ximsterm{Location} des \ximsterm{URLLink}-Objektes
    \path{/service/c102/} eingegeben werden. Für das
    \otherterm{XIMS}-Objekt
    \path{/uniweb/geschichte/personal/index.html} die
    \ximsterm{Location} \path{/geschichte/personal/}.}
	
\item {Der nächste Schritt ist das Veröffentlichen des
    departmentlinks-Verzeichnisses mitsamt den darin erstellten Links.
    Die konkrete Vorgehensweise für die Veröffentlichung von Objekten
    wird unter Punkt \ref{inhaltepublizieren} beschrieben.}
...
\item {Danach muss im \ximsterm{DepartmentRoot} \ximsterm{Container}
    ein \ximsterm{Portlet} mit der \ximsterm{Location}
...
\item {Im nächsten Schritt muss das erstellte Portlet veröffentlicht
    werden.}
...
\ximsterm{DepartmentRoot} (i.d.R. bereits vorhanden) und in weiterer
Folge weitere darin enthaltene \ximsterm{DepartmentRoot}s erstellt
...
\path{subdepartmentlinks} enthalten. Der Ordner (welcher zwingend die
Location \path{subdepartmentlinks} haben muss) enthält somit die
...
\item{Erstellen eines Portlets mit Locaton
    \path{subdepartmentlinks_portlet} im \ximsterm{DepartmentRoot} der
...
Die \ximsterm{Location} eines Dokuments in englisch und deutsch muss
...
\nolinkurl{.html.en}), d.h. Dokumente in deutscher Sprache bekommen
