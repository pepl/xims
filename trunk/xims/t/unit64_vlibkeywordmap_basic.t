...
ok( $keywrdmap );
is( $keywrdmap->document_id(), $document->document_id(), 'TestKeywordMap has correct document_id' );
is( $keywrdmap->keyword_id(), $keyword->id(), 'TestKeywordMap has correct keyword_id' );

# fetch it back by document_id
$keywrdmap = undef;
$keywrdmap = XIMS::VLibKeywordMap->new( document_id => $document->document_id(), keyword_id => $keyword_id );
