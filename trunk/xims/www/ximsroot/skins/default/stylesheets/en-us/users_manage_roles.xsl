    ...
    <html>
        ...
        <xsl:choose>
            <xsl:when test="$explicit_only = '1'">
                <a href="{$xims_box}{$goxims_users}?name={$name};manage_roles=1">Show implicitly granted roles too</a>
            </xsl:when>
            <xsl:otherwise>
                <a href="{$xims_box}{$goxims_users}?name={$name};manage_roles=1;explicit_only=1">Show explicitly granted roles only</a>
            </xsl:otherwise>
        </xsl:choose>
        <br/><br/>
...
<xsl:template name="options">
   ...
   <td colspan="3">
        <xsl:if test="$explicit_only = '1'">
            <a href="{$xims_box}{$goxims_users}?name={$name};role={name};revoke_role=1">Revoke Role Grant</a>
        </xsl:if>
    </td>
   </tr>
   </table>
