	...
	return function(editor) {
			...
			function onSubmitTableForm() {
				...
				editor.undoManager.transact(function() {
					...
					editor.dom.setAttribs(tableElm, {
						style: data.style,
						//'class': data['class']
						'class': data['colour'] + " " + data['datatable']
					});
					
					var reg = new RegExp(/^\d+$/);
					if(reg.test(data.border)){
						data.border = data.border + 'px solid';
					}
			...
			if (isProps === true) {
				...
				if (tableElm) {
					data = {
						...
						height: 'auto', //removePxSuffix(dom.getStyle(tableElm, 'height') || dom.getAttrib(tableElm, 'height')),
						...
						datatable: findClasses($(tableElm), ['datatable']) || '',
						colour: findClasses($(tableElm), ['table-orange', 'table-blue', 'table-grey']) || '',
					};
				...
				items: [
					{
						...
						items: (editor.settings.table_appearance_options !== false) ? [
							colsCtrl,
							rowsCtrl,
							{label: 'Width', name: 'width', maxLength: 4},
							/*{label: '', type: 'label'},
							{label: 'Cell spacing', name: 'cellspacing'},*/
							{label: 'Cell padding', name: 'cellpadding'},
							{label: 'Border', name: 'border', size: 20, maxLength: 20},
							{label: 'Table caption', name: 'caption', type: 'checkbox'}
						] : [
							colsCtrl,
							rowsCtrl,
					...
					//classListCtrl
					{
						...
						values: [
							{text: 'default', value: ''},
							{text: 'orange', value: 'table-orange'},
							{text: 'blue', value: 'table-blue'},
							{text: 'grey', value: 'table-grey'}
						]
					}/*,
					// temporary disabled
					{
						label: 'Activate searching, sorting, pagination',
						name: 'datatable',
						type: 'listbox',
						//text: 'None',
						values: [
							{text: 'no', value: ''},
							{text: 'yes', value: 'datatable'}
						]
					}
					*/
				]
		...
		self.row = function() {
				...
				editor.undoManager.transact(function() {
					...
					each(rows, function(rowElm) {
						...
						if (toType != rowElm.parentNode.nodeName.toLowerCase()) {
							...
							if (oldParentElm.nodeName.toLowerCase() == 'thead'){
			...
			each('left center right justify'.split(' '), function(name) {
				if (editor.formatter.matchNode(rowElm, 'alignright')) {
					data.align = name;
				}
			});
...
define("tinymce/tableplugin/Plugin", [
	...
	PluginManager.requireLangPack('table');
