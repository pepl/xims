...
tinymce.PluginManager.add('media', function(editor, url) {
			...
			body: [
				{
					...
					items: [
						...
						{name: 'medialayout', type: 'listbox', label:'Select layout',
							values: [
							         {text: 'Embed', value: 'embed-media'},
							         {text: 'Open in popup window', value: 'popup-media'}
							]
						}
					]
				},
		...
		if (data.embed) {
			...
			if (data.type == "iframe") {
				...
				if (editor.settings.audio_template_callback) {
					...
					html += (
						'<audio class="'+data.medialayout+'" controls="controls" src="' + data.source1 + '">' +
				...
				if (editor.settings.video_template_callback) {
					...
					html = (
						'<video class="'+data.medialayout+'" width="' + data.width + '" height="' + data.height + '"' + (data.poster ? ' poster="' + data.poster + '"' : '') + ' controls="controls">\n' +
	...
	function htmlToData(html) {
		...
		if(elm && (elm.tagName == 'VIDEO' || elm.tagName=="AUDIO")){
			if(elm.getAttribute('data-mce-p-class').indexOf('popup-media') != -1){
				data.medialayout = 'popup-media';
			}
			else if(elm.getAttribute('data-mce-p-class').indexOf('embed-media') != -1){
				data.medialayout = 'embed-media';
			}
		}
		/*
		if(elm.className.indexOf('popup-media') != -1){
			alert("popup");
			data.medialayout = 'popup-media';
		}
		else if(elm.className.indexOf('embed-media') != -1){
			data.medialayout = 'embed-media';
		}*/
		return data;
	}
