...
//custom IE 9 detection because of Bug when pressing Enter
function isIE9StdMode() {
	var a;
	try
	{
...
//var internetExplorer = document.selection && window.ActiveXObject && /MSIE/.test(navigator.userAgent) && !(ie9_stdMode);
var internetExplorer = document.selection && window.ActiveXObject && /MSIE/.test(navigator.userAgent);
//var oldIE = internetExplorer;
//var oldIE = internetExplorer && /MSIE\s+[4-8]\b/;
var oldIE = internetExplorer && /MSIE\s+[4-8]\b/.test(navigator.userAgent) && !(isIE9StdMode());
...
// On gecko, when the last BR of the document does not have text
// behind it, the cursor can not be put on the line after it. This
// makes pressing enter at the end of the document occasionally do
// nothing (or at least seem to do nothing). To work around it, this
// function makes sure the document ends with a span containing a
...
var Editor = (function(){
  ...
  Editor.prototype = {
    ...
    getCode: function() {
	  ...
	  // On gecko, don't count last (empty) line if the geckoLastLineHack BR is present
      if (gecko && this.container.lastChild.hackBR)
	  {
        accum.pop();
	  }
	  geckoLastLineHack(this.container);
    ...
    firstLine: function() {
      ...
      if (first) first = startOfLine(first);
      //if (first && first.hackBR) first = startOfLine(first.previousSibling);
      return first;
    },
    ...
    keyDown: function(event) {
	  ...
	  try{
			//gecko/Firefox Newline Hack: on editor init, when first <br> does not get deleted, delete it manually
			if(gecko && isBR(this.container.firstChild) && this.container.childNodes.length > 0 )
			{
    ...
    keyUp: function(event) {
	  ...
	  /*//gecko/Firefox Newline Hack: on editor init, when first <br> does not get deleted, delete it manually
	  if(gecko && isBR(this.container.firstChild) && this.container.childNodes.length > 1 )
	  {
		  //alert(this.container.firstChild.nodeName);
		  try{
			  this.container.removeChild(this.container.firstChild);
		  }
		  catch(e)
    ...
    cursorActivity: function(safe) {
	  ...
	  //if (internetExplorer) {
      //if (internetExplorer && !(ie9_stdMode)) {
	  if(oldIE) {
