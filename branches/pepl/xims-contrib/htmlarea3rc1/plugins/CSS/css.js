...
function CSS(editor, params) {
	...
	for (var i = combos.length; --i >= 0;) {
		...
		var css_class = {
			...
			refresh    : function(editor) { self.updateValue(editor, this, combo.context, combo.updatecontextclass); },
...
CSS.prototype.onSelect = function(editor, obj, context, updatecontextclass) {
	...
	// retrieve context parent element of the selection
	if ( context && updatecontextclass && parent && parent.tagName.toLowerCase() != context) {
		while ( parent = parent.parentNode ) {
			if ( typeof parent.tagName != "undefined" && parent.tagName.toLowerCase() == context ) {
				break;
			}
		}
	}
...
CSS.prototype.updateValue = function(editor, obj, context, updatecontextclass) {
	var select = editor._toolbarObjects[obj.id].element;
	var parent = editor.getParentElement();
	// retrieve context parent element of the selection
	var contextparent = parent;
	if ( context && updatecontextclass && typeof contextparent.tagName != "undefined" && contextparent.tagName.toLowerCase() != context) {
		
		while ( contextparent = contextparent.parentNode ) {
			if ( typeof contextparent.tagName != "undefined" && contextparent.tagName.toLowerCase() == context ) {
