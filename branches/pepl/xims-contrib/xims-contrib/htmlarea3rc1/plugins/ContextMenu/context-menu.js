...
//			break;
			...
			elmenus.push(null,
				     [ i18n["Modify Link"],
				       function() { editor._createLink(link); },
				       i18n["Current URL is"] + ': ' + link.href,
				       config.btnList["createlink"][1] ],

				     [ i18n["Check Link"],
                       function() {
						 var url = editor.stripBaseURL(link.href);
                         if ( url.match(/^\//) != null ) {
                            var url = editor.stripBaseURL(link.href);
                            window.open(editor.config.siterootPath + url);
                         }
                         else {
				     ...
				     [ i18n["Remove Link"],
				       function() {
					       if (confirm(i18n["Please confirm that you want to unlink this element."] + "\n" +
							   i18n["Link points to:"] + " " + editor.stripBaseURL(link.href))) {
