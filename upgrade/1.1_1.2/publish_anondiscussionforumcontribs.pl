...
print $term->banner( "Published AnondiscussionForum descendants (re-)publisher" );
...
my $ot  = XIMS::ObjectType->new( name => 'AnonDiscussionForum' );
my $iterator = $dp->objects( object_type_id => $ot->id(), published => 1 );
my $objectcount;
if ( defined $iterator and $objectcount = $iterator->getLength() ) {
    print "\nFound '" . $objectcount . "' published AnonDiscussionForum objects.\nGoing to (re)publish its descendants.\n\n";
}
else {
    print "\nNo AnonDiscussionForum objects found.\n";
    exit 0;
}
...
while ( my $object = $iterator->getNext() ) {
    ...
    my $descendantcount;
    my $updateddescendants;
    if ( defined $descendants and $descendantcount = $descendants->getLength() ) {
        print "\nFound '" . $descendantcount . "' descendants...";
        while ( my $descendant = $descendants->getNext() ) {
            if ( $descendant->publish( User => $user ) ) {
                $updateddescendants++;
            }
        }
        print "published '$updateddescendants' of them.\n";
        $updated++ if $updateddescendants == $descendantcount;
    }
    else {
...
print qq*
    AnondiscussionForum descendants publishing report:
        Total published AnondiscussionForums    $objectcount
        Updated published AnondiscussionForums  $updated
